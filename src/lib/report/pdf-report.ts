...
export function buildReportFilename(url: string, crawledAt: string): string {
  ...
  return `rankai-report-${host}-${date}.pdf`;
}
...
function drawPageHeader(ctx: ReportContext) {
  ...
  const left = sanitizeForFont('RankAI GEO + AEO Report', ctx.support.bold);
...
function drawCover(ctx: ReportContext, analysis: SiteAnalysis) {
  drawTextBlock(ctx, 'RankAI Analysis Report', {
    font: 'bold',
    size: 24,
    color: COLOR_BRAND,
    lineHeight: 30,
...
function drawFooter(page: PDFPage, fonts: PdfFonts, support: FontSupport, pageNumber: number, totalPages: number) {
  ...
  const left = sanitizeForFont('Generated by RankAI', support.body);
  page.drawText(left, {
    x: PAGE_MARGIN_X,
    y: footerY,
    font: fonts.body,
...
export async function generateAnalysisReportPdf(
  ...
  doc.setTitle(`RankAI Report - ${safeUrlHost(analysis.url)}`);
  doc.setAuthor('RankAI');
  doc.setCreator('RankAI');
  doc.setSubject('GEO and AEO analysis report');
  doc.setKeywords(['rankai', 'geo', 'aeo', 'report']);
